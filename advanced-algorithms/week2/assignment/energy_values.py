...
class Vector(object):
    ...
    def __add__(self, other):
        if type(other) == Vector:
            ...
            newList = [self.coordinates[i] + other.coordinates[i]
                       for i in range(self.dimension)]
            return Vector(newList)
        if type(other) == int or type(other) == float:
            newList = [self.coordinates[i] +
                       other for i in range(self.dimension)]
            return Vector(newList)
    ...
    def __rsub__(self, other):
        if type(other) == int or type(other) == float:
            newList = [other - self.coordinates[i]
                       for i in range(self.dimension)]
            return Vector(newList)
...
class LinearSystem(object):
    ...
    def __str__(self):
        ...
        temp = ['Equation {}: {}'.format(i + 1, p)
                for i, p in enumerate(self.equations)]
        ret += '\n'.join(temp)
        return ret
    ...
    def solve(self):
        ...
        if not triangle:
            return self.NO_UNIQUE_SOLUTIONS_MSG
        for i in range(triangle.dimension):
            if not triangle[i].valid():
                return self.NO_UNIQUE_SOLUTIONS_MSG
...
class Lineq(object):
    ...
    def __init__(self, normal_vector, constant_term=0):
        try:
            ...
            raise AssertionError(
                "The normal vector must be a list, tuple or vector object")
        try:
    ...
    def __add__(self, other):
        if type(other) == Lineq:
            try:
                ...
                raise AssertionError(
                    "The dimensions of the equations must be the same")
    ...
    def __eq__(self, other):
        ...
        s = 0
        for i in range(self.dimension):
            s += other.normal_vector.coordinates[i] * \
                self.basepoint.coordinates[i]
    ...
    def parallel(self, other):
        ...
        if self.dimension != other.dimension:
            raise TypeError(
                "The dimensions of the equations should be the same")
